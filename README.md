...
2.  GOTO  http://api.openweathermap.org => “subscribe” current weather data ==> get api key for Free version ==> create new account and Verify your emailId( Must verify to avoid issues) => go to My APi keys under your account name(top right corner) or  => save the key/appid somewhere. Now proceed further
...
3. Axios POST request assignment
            ...
            2. Pick a memeId you want (Eg 129242436) https://api.imgflip.com/caption_imagefor the POST request
            3. Create a Post request () with only query params. Following are the params (copy username and password exactly as given below):
            template_id <meme_id>
